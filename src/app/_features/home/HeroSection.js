...
function HeroSection() {
  ...
  return (
    <div className="w-full h-fit bg-gray-800 rounded-lg overflow-hidden relative">
      <Carousel>
        <CarouselContent>
          <CarouselItem>
            {heroData && (
              <>
                ...
                />
                <div className="absolute top-44 left-35 text-white w-[404px] h-[264px] opacity-100 gap-4 flex flex-col">
                  <p className="font-inter font-normal text-[16px] leading-[24px]">
                    Now Playing:
                  </p>
                  <h1 className="font-inter font-extrabold text-[36px] leading-[40px]">
                    {heroData.title}
                  </h1>
                  <div className="flex items-center gap-2">
                    <StarIcon /> {heroData.vote_average}/10
                  </div>
                  <div>{heroDataDetails?.overview}</div>

                  {!showTrailer && (
                    <button
                      className="w-[145px] h-[40px] rounded-md gap-2 flex items-center justify-center 
